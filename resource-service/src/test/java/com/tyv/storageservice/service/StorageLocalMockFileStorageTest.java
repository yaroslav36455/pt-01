...
public class StorageLocalMockFileStorageTest {
    @Autowired
    ...
    @DisplayName("Если произошла ошибка сохранения файла, то ресурс в базе данных тоже должен отсутствовать")
    void whenFileSavingWithError_thenDatabaseResourceNotBeSaved() throws IOException, URISyntaxException {
        Metadata metadata = new Metadata(Bucket.COMMENT, Category.IMAGE);

        Mockito.doReturn(Mono.error(() -> new RuntimeException("Something went wrong")))
                .when(fileStorage)
                .save(Mockito.any(StorageResource.class), Mockito.any(FilePart.class));
